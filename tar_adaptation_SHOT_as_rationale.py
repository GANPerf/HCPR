...
def train_target(args):
    ...
    while iter_num < real_max_iter:
        ...
        #*******************
        if iter_num % (interval_iter * 10) == 0 and args.cls_par > 0 and count <= 0:   # 7
            netF.eval()
            netB.eval()
            count += 1
            ...
            if count > 0:
                ...
                mem_label, retrain_input_all_w, retrain_input_all_s1,retrain_input_all_s2, retrain_feat_all, retrain_fea_all, retrain_idx_all, retrain_pseudo_all, retrain_label_all, retrain_output_all = obtain_label(dset_loaders['target'], netF, netB, netC, args, iter_num, interval_iter)
...
def obtain_label(loader, netF, netB, netC, args, iter_num, interval_iter):
    start_test = True
    with torch.no_grad():
        iter_test = iter(loader)
    ...
    ###########################
    idx_np = all_idx.int().cpu().numpy()
    idx_dd = np.hstack((dd, idx_np.reshape(-1, 1)))
    start = True
    for k in range(initc.shape[0]):
        sub_idx = np.where(predict.astype('int') == k)
        sub_dd = np.hstack((idx_dd[sub_idx, k].reshape(-1, 1), idx_dd[sub_idx, -1].reshape(-1, 1)))

        num_center = int(len(sub_idx) * 0.02)  # 0.25 for office-home; 0.02 for DimainNet-126
        ind = np.argpartition(sub_dd[:, 0], num_center)[num_center:]
        idx_class = sub_dd[:, 1][ind]

        idx_class_tensor = torch.from_numpy(idx_class)
        predict_tensor = torch.from_numpy(predict.astype('int'))
        if start:
            retrain_input_all_w, retrain_input_all_s1, retrain_input_all_s2, \
            retrain_feat_all, retrain_fea_all, retrain_output_all, retrain_idx_all, \
            retrain_pseudo_all, retrain_label_all = all_input_w[idx_class, :, :, :], all_input_s1[idx_class, :, :,
                                                                                     :], all_input_s2[idx_class, :, :,
                                                                                         :], all_feat[idx_class, :], \
                                                    all_fea_copy[idx_class, :], all_output[idx_class,
                                                                                :], idx_class_tensor, predict_tensor[
                                                        idx_class], all_label[idx_class],
            start = False
        else:
            retrain_input_all_w = torch.cat((retrain_input_all_w, all_input_w[idx_class, :, :, :]), 0)
            retrain_input_all_s1 = torch.cat((retrain_input_all_s1, all_input_s1[idx_class, :, :, :]), 0)
            retrain_input_all_s2 = torch.cat((retrain_input_all_s2, all_input_s2[idx_class, :, :, :]), 0)
            retrain_feat_all = torch.cat((retrain_feat_all, all_feat[idx_class, :]), 0)
            retrain_fea_all = torch.cat((retrain_fea_all, all_fea_copy[idx_class, :]), 0)
            retrain_output_all = torch.cat((retrain_output_all, all_output[idx_class, :]), 0)
            retrain_idx_all = torch.cat((retrain_idx_all, idx_class_tensor), 0)
            retrain_pseudo_all = torch.cat((retrain_pseudo_all, predict_tensor[idx_class]), 0)
            retrain_label_all = torch.cat((retrain_label_all, all_label[idx_class]), 0)
...
if __name__ == "__main__":
    ...
    for i in range(len(names)):
        ...
        folder = "/data/yangyang/AaD_SFDA/data/"#"./data/"#"/fast/fast_yangyang/yangyang/SHOT/object/data/"
